...
if __name__ == "__main__":
    ...
    kpts = np.array([
        [10, 10],
        [12, 12],  # close to [10, 10], should be suppressed
        [50, 50],
        [100, 100]
    ])
    ...
    # Define image shape (Height, Width)
    height, width = 120, 120
    shape = (height, width)  # CORRECT shape order for nms_fast

    # Run NMS
    selected_indices = nms_fast(kpts, scores, shape, dist_thresh=5)
    selected_kpts = kpts[selected_indices]
