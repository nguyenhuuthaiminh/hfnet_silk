...
def predict(ret, config):
    ...
    if k is not None and k > 0:
        ...
        keypoints = keypoints[idx] # [y, x]
        #convert to [x, y]
        keypoints = keypoints.flip(-1)# [N, 2] --> [x, y]
    ...
    scale = torch.tensor([W_desc - 1,H_desc - 1], 
                         device=scores.device, 
                         dtype=torch.float32)
    scale /= torch.tensor([W_img - 1,H_img - 1], 
                          device=scores.device, 
                          dtype=torch.float32)
